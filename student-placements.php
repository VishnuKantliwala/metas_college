    ...
    <section id="about" class="divider parallax layer-overlay overlay-light " data-bg-img="images/bg/metasLogo.jpg">
      <div class="container mt-50 pb-70 pt-0">
        <div class="section-content">
          <div class="row mt-10">
            <?
            $sqlPlacements = $cn->selectdb("SELECT page_name, page_desc from tbl_page WHERE page_parent_id = $page_id ORDER BY recordListingID");
            if( $cn->numRows($sqlPlacements) > 0 )
            {
                while($rowPlacements = $cn->fetchAssoc($sqlPlacements))
                {
                    extract($rowPlacements);
            ?>
            <div class="col-sm-12 col-md-12 mb-sm-20 wow fadeInUp text-center" data-wow-duration="1s" data-wow-delay="0.5s">
