            ...
            <div class="section-content">
                <div class="row mt-10">
                    ...
                    <div class="col-sm-12 col-md-12 mb-sm-20 wow fadeInUp" data-wow-duration="1s"
                        data-wow-delay="0.5s">
                        <?
                        $sqlExam = $cn->selectdb("select pdf_file, exam_name from tbl_exam order by recordListingID");
                        if( $cn->numRows($sqlExam) > 0 )
                        {
                            while($rowExam = $cn->fetchAssoc($sqlExam))
                            {
                                extract($rowExam);
                        ?>
                        <div class="col-md-4 pb-10">
                            <a target="_BLANK" download style="width:100%" href="exam_pdf/<?echo $pdf_file?>" class="btn btn-gray btn-theme-colored btn-circled btn-xl"><?echo $exam_name ?> <i class="fa fa-download"></i></a>
                        </div>
                        <?
                            }
                        }
