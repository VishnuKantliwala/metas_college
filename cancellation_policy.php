...
?>
  ...
  <div class="main-content bg-lighter">
    ...
    <?
    $sqlCancellationPolicy = $cn->selectdb('SELECT `policy` FROM tbl_policy where policy_id = 1');
    if( $cn->numRows($sqlCancellationPolicy) > 0 )
    {
        $rowCancellationPolicy = $cn->fetchAssoc($sqlCancellationPolicy);
        extract($rowCancellationPolicy);
    }
    else
    {
        $policy = "";
