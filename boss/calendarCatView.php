...
$result = $cn->selectdb("SELECT cat_id,cat_parent_id,cat_name,cat_description,cat_image FROM tbl_calendar_category where cat_parent_id=".$parentid." order by cat_id desc");
...
{   
    ...
    $var="";
    //write your query      
    $sql="select * from tbl_calendar_category";
...
$i=2;
$sql1=$con->selectdb("SELECT * FROM tbl_calendar_category");												
						 //print_r($sql1);
						...
						|| ($_FILES["imageName"]["type"][$i] == "image/bmp"))
						  ...
						  if ($_FILES["imageName"]["error"][$i] > 0)
							...
							{	
									...
									move_uploaded_file($_FILES["imageName"]["tmp_name"][$i],"../calendarcategory/big_img/" . $name4);
									
										
									make_thumb("../calendarcategory/big_img/".$name4,"../calendarcategory/".$name4,500,$extension);
...
$architect_id = trim($allDataInSheet[$i]["E"]);
	...
	$query = "SELECT cat_id FROM tbl_calendar_category WHERE cat_id = ".$cat_id;
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_calendar_category SET cat_parent_id = '".$cat_parent_id."', cat_name = '".$cat_name."',cat_image = '".$cat_image."',architect_id = '".$architect_id."' WHERE cat_id = ".$cat_id.";");
		...
		$insertTable= $con->selectdb("INSERT INTO tbl_calendar_category (cat_id,cat_parent_id, cat_name, cat_image,architect_id) VALUES ('".$cat_id."','".$cat_parent_id."', '".$cat_name."', '".$cat_image."','".$architect_id."');");
...
function delmultirec($id)
{
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_calendar SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");
    ...
    $sql = "DELETE FROM tbl_calendar_month 
            WHERE cat_id IN (" . implode(',', $categories) . ")";

    dbQuery($sql);
    
    $sql = "DELETE FROM tbl_calendar_category 
            WHERE cat_id IN (" . implode(',', $categories) . ")";
...
function deleteImage()
{
	...
	$sql = "UPDATE tbl_calendar_category
			SET cat_image = ''
			WHERE cat_id = $catId";
	dbQuery($sql);        
...
function _deleteImage($id)
{
    ...
    if (dbNumRows($result)) {
        while ($row = dbFetchAssoc($result)) {
			...
			$deleted = @unlink("../calendarcategory/". $row['cat_image']);	
			@unlink("../calendarcategory/big_img/". $row['cat_image']);	
        }	
        $deleted = true;
    }
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Calendar Category View</h4>
                                <?php
                                    if(isset($_POST['delete']))
                                    {
                                        ...
                                        for($i=0;$i<$cnt;$i++)
                                        {
                                            ...
                                            delmultirec($del_id);
                                            // $sql=  $cn->selectdb("select * from tbl_calendar_category where cat_id=$del_id");
                                            // while($row = mysqli_fetch_assoc($sql))
                                            // {
                                            //     //image
                                            //     @unlink('../calendarcategory/big_img/'.$row['cat_image']);
                                            //     @unlink('../calendarcategory/'.$row['cat_image']);
                                                
                                            // }
                                            
                                            //echo "<script>alert('".$del_id."');</script>";
                                            //$query="delete from product where product_id=".$del_id;
                                            // $con->selectdb("delete from `tbl_calendar_category` where cat_id=".$del_id);
                                        }
                                        echo "<script>window.open('calendarCatView.php','_SELF')</script>";
                                    }
                                ?>
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row mt-2 mb-2">
                                        <div class="col-lg-12">
                                            <a href="calendarCategory.php?parentid=<?php echo $parentid; ?>" class="btn btn-success m-b-sm">Add Category</a>
								            <a href="sorting_calendar_category.php" class="btn btn-success m-b-sm">Sorting</a>
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap">
                                                ...
                                                if (mysqli_num_rows($result) > 0) 
                                                {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><a href="calendarCatView.php?parentid=<?php echo $cat_id; ?>"><?php echo $cat_name; ?></a></td>
                                                        
                                                        <td><a href='calendarCategory_copy.php?id=<?php echo $cat_id ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-copy"></i></a></td>
                                                        <td><a href='calendarcategory_up.php?category_id=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
                                                        <td><a href='deleteCalendarCat.php?catId=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
