...
if(isset($_POST['updateSlider']))
{
            ...
            $con->insertdb("UPDATE `tbl_news` SET `news_name` = '".$news_name."', `description` = '".$description."', `link_url` = '".$link_url."' WHERE `tbl_news`.`news_id` = '".$news_id."'");
...
if(isset($_GET["Image"]))
	...
	$page=$_GET['page'];
	$news_id= $_GET['news_id'];
	$records=$con->selectdb("SELECT * FROM tbl_news where news_id=".$news_id."");
	while($row=mysqli_fetch_row($records))
	{
	  unlink('../news/'.$row[2]);
	  unlink('../news/big_img/'.$row[2]);

	}
	$con->selectdb("update tbl_news set image_title='' where news_id = '".$news_id."'");
	header("location: newsUpdate.php?news_id=".$news_id."&page=".$page);
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-12">
                                            <input type="text" class="form-control" id="news_name" name="news_name"
                                                placeholder="News Name" value="<? echo $row['news_name']; ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group" style="display:none">
                                        ...
                                        <div class="col-sm-4">
                                            <input type="file" id="image_title" name="image_title" class="dropify"
                                                ...
                                                ../news/".$row['image_title'];}?>" />

                                            <? if($row['image_title']!=''){?>
                                            <!-- <br><img src="../news/<? //echo $row['image_title'];?>" width="100"> -->
                                            <a href="newsUpdate.php?news_id=<?php echo $row['news_id']; ?>&Image=Del"
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-offset-2 col-sm-10">
                                            ...
                                            <button type="submit" name="myButton" id="myButton"
                                                class="btn btn-lighten-danger"
                                                onClick="window.location.href='newsView.php'; return false;">Cancel</button>
                                        </div>
                                    </div>
