...
$query = $con->selectdb('SELECT * FROM tbl_calendar_category');
...
$sqlC=$con->selectdb("SELECT cat_id FROM tbl_calendar where calendar_id=".$calendar_id."");
    ...
    function generate_menu($parent)
    {
            ...
            foreach($menuItems as $key => $value)
            {
                ...
                {    
                    ...
                    {
                        echo '<li data-jstree='.'{"opened":true}'.'><a href="calendarcategory.php?id=' . $key . '">' . $value['cat_name'] . '<b class="caret"></b></a>';
                    }
                    else if($value['cat_parent_id'] != 0 && in_array($key, $parentMenuIds))
                    {
                        echo '<li data-jstree='.'{"opened":true}'.'><a href="calendarcategory.php?id=' . $key . '">' . $value['cat_name'] . '</a>';
                    }
                    else
                    {
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">calendar Form</h4>
                                
                                <form class="form-horizontal" method="post" action="calendar_upload.php" id="myform" name="myform" enctype="multipart/form-data">
                                    <input type="hidden" name="page" id="page" value="<? echo isset($_GET['page']);?>">

                                    <?php
                                    $records=$cn->selectdb("SELECT * FROM tbl_calendar where calendar_id=".$calendar_id."");
                                    while($row=mysqli_fetch_array($records))
                                    {
                                    ?>
                                    <input type="hidden" name="calendar_id" id="calendar_id" value="<?php echo $row[0]; ?>" />
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-12">
                                            <input type="date" class="form-control" id="date" name="from_date" placeholder="Date" value="<?echo $row['from_date']?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-12">
                                            <input type="date" class="form-control" id="date" name="to_date" placeholder="Date" value="<?echo $row['to_date']?>">
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <label for="inputEmail3" class="col-sm-12 control-label">calendar Name</label>
                                        <div class="col-sm-12">
                                            <input type="text" class="form-control" id="calendar_name" name="calendar_name" value="<?php echo $row['calendar_name']; ?>" placeholder="calendar Name">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-offset-2 col-sm-10">
                                            <button type="submit" name="updateCalendar" id="updateCalendar" class="btn btn-success">Update</button>
                                            <button type="submit" name="myButton" id="myButton" class="btn btn-lighten-danger" onClick="window.location.href='calendarView.php'; return false;">Cancel</button>
                                        </div>
                                    </div>
