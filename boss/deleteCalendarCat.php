...
//print_r($_GET);die;
	...
	$sql = "SELECT cat_id,calendar_id,calendar_image
	        FROM tbl_calendar
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_calendar_category SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");
				...
				elseif($cntCatId== 1)
				{
					...
					// delete the products
					$sqlD = "DELETE FROM tbl_calendar_category
