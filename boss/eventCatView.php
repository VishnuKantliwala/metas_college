...
else
{
    ...
    $result = $cn->selectdb("SELECT cat_id,cat_parent_id,cat_name,cat_description,cat_image FROM tbl_eventcategory where cat_parent_id=".$parentid." order by cat_id desc");
...
{   
    ...
    $var="";
    //write your query      
    $sql="select * from tbl_eventcategory";
...
$architect_id = trim($allDataInSheet[$i]["E"]);
	...
	if($existid>0) {
		...
		$insertTable= $con->selectdb("UPDATE tbl_eventcategory SET cat_parent_id = '".$cat_parent_id."', cat_name = '".$cat_name."',cat_image = '".$cat_image."',architect_id = '".$architect_id."' WHERE cat_id = ".$cat_id.";");
		...
		$insertTable= $con->selectdb("INSERT INTO tbl_eventcategory (cat_id,cat_parent_id, cat_name, cat_image,architect_id) VALUES ('".$cat_id."','".$cat_parent_id."', '".$cat_name."', '".$cat_image."','".$architect_id."');");
...
function delmultiblogcat($id)
{
	...
	$sql = "SELECT cat_id,event_id,event_image,multi_images
	        FROM tbl_events
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_events SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");

				}
				elseif($cntCatId== 1)
				{
					$image_list = explode(',',$row['event_image']);
					...
					foreach($image_list1 as $rowF1)
					{
						//print_r($image_list);die;
						$new_image_list1 = '';
						unlink('../eventF/big_img/'.$rowF1);
						unlink('../eventF/'.$rowF1);
					}
					// end of multiple images
					
					// delete the products
					$sqlD = "DELETE FROM tbl_events
    ...
    $sql = "DELETE FROM tbl_eventcategory 
            WHERE cat_id IN (" . implode(',', $categories) . ")";
    dbQuery($sql);
...
function _deleteImage($id)
{
    ...
    if (dbNumRows($result)) {
        while ($row = dbFetchAssoc($result)) {
			...
			$deleted = @unlink("../eventcategory/". $row['cat_image']);	
			@unlink("../eventcategory/big_img/". $row['cat_image']);	
        }	
        $deleted = true;
    }
    ...
    <div id="wrapper">
        ...
        <div class="navbar-custom">
            ...
            <ul class="list-unstyled topnav-menu topnav-menu-left m-0">
                ...
                <li>
                    <h4 class="page-title-main">Event Category</h4>
                </li>
            ...
            <div class="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h4 class="mt-0 header-title">Event Category View</h4>
                                <?php
									...
									for($i=0;$i<$cnt;$i++)
									{
										...
										echo "<script>window.open('eventCatView.php','_SELF')</script>";
									}
								?>
                                ...
                                <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <a href="eventcategory.php?parentid=<?php echo $parentid; ?>" class="btn btn-success m-b-sm mt-2 mb-2">Add Category</a>
                                            <a href="sorting_event_category.php" class="btn btn-success m-b-sm mt-2 mb-2">Sorting</a>
									...
									<tfoot>
										...
										if (mysqli_num_rows($result) > 0) 
										{
                                            ...
                                            <tr>
												...
												<td><a href="eventCatView.php?parentid=<?php echo $cat_id; ?>"><?php echo $cat_name; ?></a></td>
												<td><?php if($cat_image!=''){?><img src='../eventcategory/<?php echo $cat_image; ?>' height="100" width="100"><?php } else { echo "No Image"; }?></td>
                                                <td><a href='eventcategory_up.php?category_id=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>'><i class="fa fa-edit"></i></a></td>
												<td><a href='deleteevent_cat.php?catId=<?php echo $cat_id; ?>&page=<? echo isset($_GET['page']);?>' onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-trash"></i></a></td>
