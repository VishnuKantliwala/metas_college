 ...
 // echo $parseurl['path'];die;
	  ...
	  if(isset($_POST['addProduct']))
	  {
	     // print_r($_POST);
			...
			$course_admission_process=$_POST['course_admission_process'];
			$course_application_process=$_POST['course_application_process'];
			$course_e_and_s_process=$_POST['course_e_and_s_process'];
			$course_faculty_profile_link=$_POST['course_faculty_profile_link'];
		...
		$RES = $con->insertdb("INSERT INTO `tbl_product` (
							...
							`course_e_and_s_process`, 
							`course_faculty_profile_link`
							)
							VALUES (
							'".$product_name."',
							'".$description."',  
	    ...
	    if(isset($_POST['updateProduct']))
	  	{
		   ...
		   //echo $attributeKey.' '.$_POST['mulradio'][$attributeKey].'<br>';
			...
			if($_FILES["frontimg"]['error'] > 0)// it means no new image selected insert previous one......
			{
				...
				$con->insertdb("UPDATE `tbl_product` SET product_name='".$product_name."',description='".$description."',cat_id='".$catID."',product_image='".$frontimg2."',multi_images='".$frontimg1."', meta_tag_title='".$meta_tag_title."',meta_tag_description='".$meta_tag_description."',meta_tag_keywords='".$meta_tag_keywords."',slug='".$slug."',date='".$date."',month='".$month."',year='".$year."', `course_admission_process` = '".$course_admission_process."', `course_application_process` = '".$course_application_process."', `course_e_and_s_process` = '".$course_e_and_s_process."', `course_faculty_profile_link` = '".$course_faculty_profile_link."'   where product_id='".$product_id."'");
			}
			else
			{
			...
			// if($_FILES["download_file"]["name"]=="")
			{
				...
				$con->insertdb("UPDATE `tbl_product` SET  pdf_file='".$final."'  where product_id='".$product_id."'");
			}
