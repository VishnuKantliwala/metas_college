            ...
            <div class="section-content">
                <div class="row mt-10">
                    ...
                    <div class="col-sm-12 col-md-12 mb-sm-20 wow fadeInUp" data-wow-duration="1s"
                        data-wow-delay="0.5s">
                        <?
                        $sqlIQACReport = $cn->selectdb("select pdf_file, iqac_report_name from tbl_iqac_report order by recordListingID");
                        if( $cn->numRows($sqlIQACReport) > 0 )
                        {
                            while($rowIQACReport = $cn->fetchAssoc($sqlIQACReport))
                            {
                                extract($rowIQACReport);
                        ?>
                        <div class="col-md-4 pb-10">
                            <a  download target="_BLANK" style="width:100%" href="iqac_report_pdf/<?echo $pdf_file?>" class="btn btn-gray btn-theme-colored btn-circled btn-xl"><?echo $iqac_report_name ?> <i class="fa fa-download"></i></a>
                        </div>
                        <?
                            }
                        }
